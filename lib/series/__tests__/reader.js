...
test('read from series', async () => {
	...
	expect(r0.ptr).toBe(db.read.mock.calls[0][0]);
...
test('read from a ptr', async () => {
	const db = dbFactory();
	const ptr = 42;
	db.search.mockReturnValueOnce(Promise.resolve(ptr));
	const s = await readerFactory(db.openSeries)({ptr});
	await s.read();
	expect(db.read.mock.calls[0][0]).toBe(ptr + 1);
});
