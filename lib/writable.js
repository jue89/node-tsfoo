...
const MINTS = Buffer.from([128, 0, 0, 0, 0, 0]).readIntBE(0, 6);
...
class WriteStream extends stream.Writable {
	...
	async _write (chunk, encoding, callback) {
		try {
			...
			// Check timestamp to be monotonically increasing
			if (chunk.timestamp < this.lastTs) throw new Error('timestamps must be monotonically increasing. Last timestamp: ' + this.lastTs);
