...
async function create (path, meta) {
	...
	const m = metaRead(meta);
	if (typeof m.itemLen !== 'number') throw new Error('meta must export itemLen');
	if (typeof m.pack !== 'function') throw new Error('meta must export function pack');
	if (typeof m.unpack !== 'function') throw new Error('meta must export function unpack');

	// Create required buffers
	const header = headerGen({meta, itemLen: m.itemLen});
	const data = Buffer.alloc(header.dataStart);
	header.data.copy(data, 0, 0);
	data.write(meta, header.metaStart);
