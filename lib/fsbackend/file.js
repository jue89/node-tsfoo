...
class File extends events.EventEmitter {
	...
	async openRead () {
		...
		try {
			...
			this.watchFileListener = (stats) => {
				if (stats.size === this.size) return;
				this.size = stats.size;
			};
			fs.watchFile(this.path, this.watchFileListener);
	...
	async openReadWrite ({blocking, create} = {}) {
		...
		// Open file
		this.fd = await open(this.path, (!create || exists) ? 'r+' : 'w+');
		this.state = 'rw';
	...
	async read ({offset, size, blocking, minSize} = {}) {
	...
	async cachedRead ({offset, size, blocking, readAhead} = {}) {
		assert(size !== undefined, 'Size argument is required');

		// Defaults
		if (readAhead === undefined) readAhead = 1024 * 1024;
	...
	async close () {
		if (this.state === 'ro') {
			...
			if (this.watchFileListener) {
				fs.unwatchFile(this.path, this.watchFileListener);
			};
			await close(this.fd);
