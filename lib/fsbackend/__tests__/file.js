...
describe('File::openRead', () => {
	...
	test('update size and fire event', async () => {
		...
		expect(mockFs.watch.mock.calls[0][0]).toEqual(path);
		mockFs.stat.mockImplementationOnce((p, cb) => cb(null, {size}));
		await mockFs.watch.mock.calls[0][1]();
		expect(onChange.mock.calls.length).toBe(0);
		mockFs.stat.mockImplementationOnce((p, cb) => cb(null, {size: size + 1}));
		await mockFs.watch.mock.calls[0][1]();
...
describe('File::read', () => {
	...
	test('throw error if not enough data is available', async () => {
		...
		const size = 21;
		f.size = size - 1;
		return expect(f.read({size})).rejects.toThrow('Would block');
	});
...
describe('File::cachedRead', () => {
	...
	test('use cached data', async () => {
		...
		await expect(f.cachedRead({offset: size, size: size + 1})).rejects.toThrow('Would block');
		f.size = size * 2 + 1;
...
describe('File::close', () => {
	test('close ro file', async () => {
		...
		await f.close();
		expect(mockFs.close.mock.calls[0][0]).toBe(f.fd);
		expect(mockFs.watch.mock.results[0].value.close.mock.calls[0][0]).toEqual(f.path);
		expect(f.state).toBeUndefined();
	});
