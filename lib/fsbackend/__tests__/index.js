...
describe('read()', () => {
	test('read nth block', async () => {
		...
		// Read nth block
		const n = 678;
		expect(await db.read(n, {blocking: false})).toMatchObject({timestamp, value});

		expect(mockBlockReader.mock.instances[0].readBlock.mock.calls[0][0]).toBe(n);
		expect(mockBlockReader.mock.instances[0].readBlock.mock.calls[0][1]).toMatchObject({blocking: false});
