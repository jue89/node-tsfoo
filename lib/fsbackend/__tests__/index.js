...
describe('open()', () => {
	test('create file handles', async () => {
		...
		const name = 'xyz';
		await open({dir, name});
		expect(mockFile.mock.calls[0][0].path).toEqual(`${dir}/idx-${name}`);
		expect(mockFile.mock.calls[1][0].path).toEqual(`${dir}/dat-${name}`);
	...
	test('fail if magic is not present', async () => {
		...
		expect(mockFile.prototype.close.mock.calls.length).toBe(3);
	});
});
...
describe('write()', () => {
	test('write data', async () => {
		...
		idx.writeUIntBE(mockFile.prototype.write.mock.results[1].value.offset, 6, 6);
		idx.writeUIntBE(mockFile.prototype.write.mock.results[1].value.size, 12, 4);
...
describe('close()', () => {
	test('close all files', async () => {
		const db = await open({});
		await db.close();
		expect(mockFile.prototype.close.mock.calls.length).toBe(4);
	});
});
