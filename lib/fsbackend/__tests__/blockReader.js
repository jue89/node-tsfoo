...
describe('BlockReader::bisectSearch', () => {
	...
	test('find first block of a group of same blocks', async () => {
		const offset = 0;
		const blockSize = 1;
		const data = Buffer.from([0, 1, 2, 2, 2, 3, 4]);
		const read = ({offset, size}) => data.slice(offset, offset + size);
		const file = {size: data.length, read};
		const br = new BlockReader({file, offset, blockSize});
		const needle = Buffer.from([2]);
		const fn = (b) => Buffer.compare(needle, b);
		expect(await br.bisectSearch(fn)).toBe(2);
	});

	test('find first, larger block', async () => {
		const offset = 0;
		const blockSize = 1;
		const data = Buffer.from([2, 2, 2, 3, 4]);
		const read = ({offset, size}) => data.slice(offset, offset + size);
		const file = {size: data.length, read};
		const br = new BlockReader({file, offset, blockSize});
		const needle = Buffer.from([1]);
		const fn = (b) => Buffer.compare(needle, b);
		expect(await br.bisectSearch(fn)).toBe(0);
	});
});
