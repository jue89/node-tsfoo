...
describe('writable.create', () => {
	...
	test('set lastTs to minimal TS', async () => {
		const lowerBound = Buffer.from([128, 0, 0, 0, 0, 0]).readIntBE(0, 6) - 1;
		const seriesFile = {
			fd: 123,
			header: {dataStart: 42, tsLen: 6, itemLen: 14},
			stat: {size: 42}
...
describe('writable.WriteStream', () => {
	...
	test('reject timestamps less or equal than the last one', async () => {
		...
		const onError = jest.fn();
		w.on('error', onError);
		await new Promise((resolve) => w.write({timestamp, value: 2}, resolve));
		expect(onError.mock.calls[0][0].message).toEqual('timestamps must be strictly monotonically increasing. Last timestamp: ' + timestamp);
	});
});
