...
describe('series.open', () => {
	...
	test('enforce mode to be \'r\' or \'w\'', async () => {
		try {
			await seriesOpen('test', 'r#');
			throw new Error('Failed!');
		} catch (e) {
			expect(e.message).toEqual('mode must be r, w or w+');
		}
	});
	...
	test('get non-blocking lock for \'w\'', async () => {
		const fd = 123;
		const mode = 'w';
		mockFs.open.mockImplementationOnce((f, m, cb) => cb(null, fd));
		await seriesOpen('test', mode);
		expect(mockFs.flock.mock.calls[0][0]).toBe(fd);
		expect(mockFs.flock.mock.calls[0][1]).toEqual('exnb');
	});

	test('get blocking lock for \'w+\'', async () => {
		const fd = 123;
		const mode = 'w+';
		mockFs.open.mockImplementationOnce((f, m, cb) => cb(null, fd));
		await seriesOpen('test', mode);
