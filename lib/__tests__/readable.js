...
jest.mock('../cachedRead.js');
const mockRead = require('../cachedRead.js');
...
describe('readable.create', () => {
	...
	test('open series file', async () => {
		...
		expect(r.cache).toBeInstanceOf(mockRead);
		expect(mockRead.mock.calls[0][0]).toBe(seriesFile.fd);
	});
...
describe('readable.ReadStream', () => {
	...
	test('read one item', async () => {
		const timestamp = 1;
		const value = 42;
		const chunk = Buffer.alloc(7);
		chunk.writeIntBE(timestamp, 0, 6);
		chunk.writeUInt8(value, 6);
		mockRead.prototype.read.mockReturnValueOnce(Promise.resolve(chunk));
		const seriesFile = {
			fd: 123,
			path: 'abc',
			follow: false,
		...
		r.on('data', onData);
		await immediate();
		expect(mockRead.prototype.read.mock.calls[0][0]).toBe(seriesFile.ptr);
		expect(mockRead.prototype.read.mock.calls[0][1]).toBe(seriesFile.recordLen);
	...
	test('ignore items with timestamp <= from', async () => {
		let timestamp = 1;
		const value = 42;
		mockRead.prototype.read.mockImplementation((offset, len) => {
			const buf = Buffer.alloc(len);
			buf.writeIntBE(timestamp++, 0, 6);
			buf.writeUInt8(value, 6);
