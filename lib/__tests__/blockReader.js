...
describe('BlockReader::readNext', () => {
	test('read blocks', async () => {
		...
		expect((await br.readNext({blocking: false})).length).toBe(blockSize);
...
describe('BlockReader::bisectSeek', () => {
	test('find first block', async () => {
		...
		expect(await br.bisectSeek(fn)).toBe(0);
		expect(br.ptr).toBe(0);
	});
	...
	test('find second block', async () => {
		...
		expect(await br.bisectSeek(fn)).toBe(1);
		expect(br.ptr).toBe(1);
	});
	...
	test('find third block', async () => {
		...
		expect(await br.bisectSeek(fn)).toBe(2);
		expect(br.ptr).toBe(2);
	});
	...
	test('find last block', async () => {
		...
		expect(await br.bisectSeek(fn)).toBe(3);
		expect(br.ptr).toBe(3);
	});
	...
	test('find future block', async () => {
		...
		expect(await br.bisectSeek(fn)).toBe(4);
		expect(br.ptr).toBe(4);
	});
});
