const header = require('../header.js');

const MAGIC = Buffer.from('TSF1');

describe('header.read', () => {
	const headerRead = header.read;

	test('enforce right length', () => {
		try {
			headerRead(MAGIC);
			throw new Error('FAILED!');
		} catch (e) {
			expect(e.message).toEqual('Header is too short');
		}
	});

	test('enforce magic', () => {
		try {
			headerRead(Buffer.concat([
				Buffer.from('abcd'),
				Buffer.alloc(4 * 4)
			]));
			throw new Error('FAILED!');
		} catch (e) {
			expect(e.message).toEqual('Header Magic not found: 0x61626364 != 0x54534631');
		}
	});

	test('read header data', () => {
		const offsets = Buffer.alloc(4 + 4 + 4 + 4);
		const metaStart = 0x42;
		const metaEnd = 0x55;
		const dataStart = 0xfff;
		const tsLen = 0x8;
		const itemLen = 0xf;
		offsets.writeUInt32BE(metaStart, 0);
		offsets.writeUInt32BE(metaEnd, 4);
		offsets.writeUInt32BE(dataStart, 8);
		offsets.writeUInt8(tsLen, 13);
		offsets.writeUInt16BE(itemLen, 14);
		const data = Buffer.concat([MAGIC, offsets]);
		const hdr = headerRead(data);
		expect(hdr).toMatchObject({metaStart, metaEnd, dataStart, tsLen, itemLen, data});
	});
});

describe('header.gen', () => {
	const headerGen = header.gen;

	test('expect itemLen to be set', () => {
		try {
			headerGen({metaCode: 'abc'});
			throw new Error('FAILED!');
		} catch (e) {
			expect(e.message).toEqual('itemLen must be set');
		}
	});

	test('expect metaCode to be set', () => {
		try {
			headerGen({itemLen: 0});
			throw new Error('FAILED!');
		} catch (e) {
			expect(e.message).toEqual('metaCode must be set');
		}
	});

	test('generate header', () => {
		const metaCode = 'abcdefá';
		const metaStart = 20;
		const metaEnd = metaStart + Buffer.byteLength(metaCode);
		const dataStart = metaEnd;
		const itemLen = 12;
		const tsLen = 6;
		const header = headerGen({metaCode, itemLen: 12});
		expect(header).toMatchObject({metaStart, metaEnd, dataStart, tsLen, itemLen});
		expect(header.data.slice(0, 4).toString('hex')).toEqual(MAGIC.toString('hex'));
		expect(header.data.readUInt32BE(4)).toBe(metaStart);
		expect(header.data.readUInt32BE(8)).toBe(metaEnd);
		expect(header.data.readUInt32BE(12)).toBe(dataStart);
		expect(header.data.readUInt8(17)).toBe(tsLen);
		expect(header.data.readUInt16BE(18)).toBe(itemLen);
	});
});
